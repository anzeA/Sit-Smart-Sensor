...
## Released models
Within the `models` directory, I've included three models available for testing the application. Each model has demonstrated accuracy rates exceeding 95% on my personal dataset. Among these, ResNet34 stands out as it strikes an optimal balance between accuracy and computational speed.
...
- After training, test the model using `run.explain=True` to visualize its decision-making process. This will help identify any biases or errors in the model. If you find any, consider adding more images to the dataset to improve model performance.
...
- About 100 images per class should be enough to train a model with accuracy over 95%.
